          ...
          {showComposer ? (
            ...
            <div
              ...
              onClick={() => {
                ...
                if (user?.getUid()) {
                  CometChat.unblockUsers([user.getUid()])
                    .then((response: Record<string, any>) => {
                      const result = response[user.getUid()];
                      if (result === "success") {
                        user.setBlockedByMe(false);
                        CometChatUserEvents.ccUserUnblocked.next(user);
                        console.log(`User ${user.getUid()} was successfully unblocked.`);
                      } else {
                        console.warn(`Failed to unblock user ${user.getUid()}:`, result);
                      }
                    })
                    .catch((error) => {
                      console.error(`Error unblocking user ${user.getUid()}:`, error);
                    });
                }
